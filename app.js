classesCollectionRef.doc(id).update(dataToUpdate)
    ...
    const renderCurrentView = () => {
        switch (view) {
            case 'list':
                return <div id="taskListSection" className="bg-white dark:bg-gray-800/50 rounded-2xl shadow-lg p-2 sm:p-6 mb-3 sm:mb-6 mt-3 space-y-4"> <h2 className="text-xl sm:text-2xl font-semibold text-blue-600 dark:text-blue-400 text-left mb-3 sm:mb-6">Lista de tareas</h2> {[...tasks].sort((a, b) => { if (a.completed && !b.completed) return 1; if (!a.completed && b.completed) return -1; const dateA = new Date(`${a.dueDate}T${a.dueTime || '00:00'}`); const dateB = new Date(`${b.dueDate}T${b.dueTime || '00:00'}`); return dateA - dateB; }).map(task => { const status = getTaskStatus(task.dueDate, task.dueTime, task.completed); const cardStyle = getTaskCardStyle(status, task.completed); return ( <div key={task.id} id={task.id} onClick={() => handleTaskCardClick(task)} className={`rounded-xl shadow-lg border-l-8 p-2.5 sm:p-6 transition-all duration-300 ${cardStyle.bg} ${cardStyle.border} ${cardStyle.hoverClasses} cursor-pointer`}> <div className="flex flex-col sm:flex-row sm:items-center justify-between space-y-2 sm:space-y-0"> <div className="flex items-start space-x-2.5 sm:space-x-4 flex-1"> <button onClick={(e) => { e.stopPropagation(); toggleTask(task.id); }} className={`mt-0.5 w-6 h-6 sm:w-6.5 sm:h-6.5 rounded border-2 flex items-center justify-center flex-shrink-0 border-gray-300 dark:border-gray-500 hover:border-green-500 dark:hover:border-green-400 hover:bg-green-100 dark:hover:bg-green-900/30`}></button> <div className="flex-1 min-w-0">
                    ...
                    <div className="max-w-7xl mx-auto px-3 sm:px-6">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-2">
                                ...
                                <div>
                                    <h1 className="text-3xl sm:text-3xl font-bold text-white leading-tight">GESTOR ACADÉMICO</h1>
                                    <p className="text-xs sm:text-sm text-blue-200 mt-1"><span className="font-semibold text-white">{user.email}</span></p>
                                </div>
                            </div>
...
root.render(<App />);    for (let i = 0; i < 7; i++) {
...
const AcademicTaskManager = ({ user }) => {
    ...
    const handleSaveClass = (classData) => {
        if (classData.id) { // Update existing class
            ...
            classesconst { useState, useEffect, useRef } = React;
        ...
        <div className="min-h-screen bg-blue-600 flex flex-col justify-center items-center p-4">
            <div className="max-w-md w-full mx-auto bg-white dark:bg-gray-800 rounded-2xl shadow-2xl p-4 sm:p-8 border-t-4 border-blue-600">
                {registerSuccessMessage && (
                    <div className="bg-green-100 border-l-4 border-green-500 text-green-800 p-3 mb-4 rounded-r-lg" role="alert">
                        <p className="font-bold text-sm">¡Éxito!</p>
                        <p className="text-sm">{registerSuccessMessage}</p>
                    </div>
                )}
...
const TaskModal = ({ isOpen, onClose, onSave, showAlert, taskToEdit, selectedDate }) => {
    ...
    return (
        <div className="!fixed !inset-0 !bg-black/60 !flex !items-center !justify-center !z-[9999] !p-2">
            <div className="!bg-white !rounded-3xl !shadow-2xl !p-6 sm:!p-8 !w-[99%] !max-w-2xl !mx-auto !border !border-gray-200">
                ...
                <div className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <input type="text" name="subject" placeholder="Asignatura" value={taskData.subject || ''} onChange={handleChange} className="!w-full !bg-white !text-gray-900 !border !border-gray-300 !rounded-xl !px-4 !py-3 !text-base focus:!ring-2 focus:!ring-blue-500 focus:!border-transparent" />
                        <input type="text" name="title" placeholder="Título de la tarea" value={taskData.title || ''} onChange={handleChange} className="!w-full !bg-white !text-gray-900 !border !border-gray-300 !rounded-xl !px-4 !py-3 !text-base focus:!ring-2 focus:!ring-blue-500 focus:!border-transparent" />
                    </div>
                    <select name="type" value={taskData.type || 'Tarea'} onChange={handleChange} className="!w-full !bg-white !text-gray-900 !border !border-gray-300 !rounded-xl !px-4 !py-3 !text-base focus:!ring-2 focus:!ring-blue-500 focus:!border-transparent">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <input type="date" name="dueDate" value={taskData.dueDate || ''} onChange={handleChange} className="!w-full !bg-white !text-gray-900 !border !border-gray-300 !rounded-xl !px-4 !py-3 !text-base focus:!ring-2 focus:!ring-blue-500 focus:!border-transparent" />
                        <input type="time" name="dueTime" placeholder="Hora (opcional)" value={taskData.dueTime || ''} onChange={handleChange} className="!w-full !bg-white !text-gray-900 !border !border-gray-300 !rounded-xl !px-4 !py-3 !text-base focus:!ring-2 focus:!ring-blue-500 focus:!border-transparent" />
                    </div>
                    <textarea name="description" placeholder="Descripción (opcional)" value={taskData.description || ''} onChange={handleChange} rows="3" className="!w-full !bg-white !text-gray-900 !border !border-gray-300 !rounded-xl !px-4 !py-3 !text-base focus:!ring-2 focus:!ring-blue-500 focus:!border-transparent !resize-y"></textarea>
                    <div className="flex justify-center w-full space-x-4 pt-2">
                        <button onClick={onClose} className="!bg-gray-500 !text-white !rounded-xl !px-5 !py-2.5 hover:!bg-gray-600 !text-base !font-medium">Cancelar</button>
...
const ClassModal = ({ isOpen, onClose, onSave, showAlert, classToEdit, selectedDay, selectedTime }) => {
    ...
    return (
        <div className="!fixed !inset-0 !bg-black/60 !flex !items-center !justify-center !z-[9999] !p-2">
            <div className="!bg-white !rounded-3xl !shadow-2xl !p-6 sm:!p-8 !w-[99%] !max-w-2xl !mx-auto !border !border-gray-200">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="!font-semibold !text-blue-600 !text-xl sm:!text-2xl">{isEditMode ? 'Editar Clase' : 'Agregar Nueva Clase'}</h3>
                    <button onClick={onClose} className="!text-gray-500 hover:!text-gray-700 !p-1 !rounded-full">
                        <IconClose className="w-6 h-6" />
                    </button>
                </div>
                <div className="space-y-4">
                    <input type="text" name="subject" placeholder="Asignatura" value={classData.subject || ''} onChange={handleChange} className="!w-full !bg-white !text-gray-900 !border !border-gray-300 !rounded-xl !px-4 !py-3 !text-base focus:!ring-2 focus:!ring-blue-500 focus:!border-transparent" />

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <select name="dayOfWeek" value={classData.dayOfWeek || 'Lunes'} onChange={handleChange} className="!w-full !bg-white !text-gray-900 !border !border-gray-300 !rounded-xl !px-4 !py-3 !text-base focus:!ring-2 focus:!ring-blue-500 focus:!border-transparent">
                            {daysOfWeek.map(day => <option key={day} value={day}>{day}</option>)}
                        </select>
                        {/* Changed to input type="time" for manual input */}
                        <input type="time" name="startTime" placeholder="Hora de inicio" value={classData.startTime || ''} onChange={handleChange} className="!w-full !bg-white !text-gray-900 !border !border-gray-300 !rounded-xl !px-4 !py-3 !text-base focus:!ring-2 focus:!ring-blue-500 focus:!border-transparent" />
                    </div>
                    <input type="time" name="endTime" placeholder="Hora de fin (opcional)" value={classData.endTime || ''} onChange={handleChange} className="!w-full !bg-white !text-gray-900 !border !border-gray-300 !rounded-xl !px-4 !py-3 !text-base focus:!ring-2 focus:!ring-blue-500 focus:!border-transparent" />
                    <textarea name="description" placeholder="Descripción (opcional)" value={classData.description || ''} onChange={handleChange} rows="3" className="!w-full !bg-white !text-gray-900 !border !border-gray-300 !rounded-xl !px-4 !py-3 !text-base focus:!ring-2 focus:!ring-blue-500 focus:!border-transparent !resize-y"></textarea>
                    <div className="flex justify-center w-full space-x-4 pt-2">
                        <button onClick={onClose} className="!bg-gray-500 !text-white !rounded-xl !px-5 !py-2.5 hover:!bg-gray-600 !text-base !font-medium">Cancelar</button>
                        <button onClick={handleSubmit} className="!bg-blue-600 !text-white !rounded-xl !px-5 !py-2.5 hover:!bg-blue-700 !flex !items-center !justify-center !space-x-2 !text-base !font-medium">
                            {isEditMode ? <IconCheck width="18" height="18" /> : <IconPlus width="18" height="18" />}
