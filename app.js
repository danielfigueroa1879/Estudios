...
const AcademicTaskManager = lazy(() => Promise.resolve(
    ({ user }) => {
        ...
        const classesCollectionRef = db.collection('users').doc(user.uid).collection('classes');
...
// --- Main App Component (Actualizado con lógica de instalación y Lazy Loading) ---
const App = () => {
    ...
    // Renderiza el spinner de carga inicial si la autenticación aún está pendiente
    // Esto se mantiene para la carga inicial de Firebase, antes de que se sepa si el usuario está logueado o no.
    if (loading) {
        return (
            <div className="min-h-screen bg-gray-100 dark:bg-gray-900 flex items-center justify-center">
                <IconSpinner /> <span className="text-xl ml-4 text-gray-600 dark:text-gray-300">Cargando la aplicación...</span>
            </div>
        );
    }
    ...
    return (
        <div>
            {/* Suspense muestra un fallback (el spinner) mientras los componentes lazy se cargan */}
            <Suspense fallback={
                <div className="min-h-screen bg-gray-100 dark:bg-gray-900 flex items-center justify-center">
                    <IconSpinner /> <span className="text-xl ml-4 text-gray-600 dark:text-gray-300">Cargando componentes...</span>
                </div>
            }>
                {user ? (
